...
const GREMLIN_SPEED = 1.8;     // units/sec (wave 1)
const GREMLIN_SPEED_INC = 0.3; // per wave
const PROJ_SPEED = 24;         // units/sec
const PICKUP_R = 2.5;          // heart collection radius
const GREMLIN_HIT_R = 1.4;     // projectile hit radius
const GIANT_ATTACK_R = 2.9;    // gremlin reaches giant heart
const GIANT_HP_MAX = 100;
const GREMLIN_DMG = 25;
const LOVE_PER_HEART = 20;
const HEART_INTERVAL = 2.0;
const GREMLIN_INTERVAL_BASE = 3.5;
...
function initGS(): GS {
  return {
    ...
    giantHp: GIANT_HP_MAX, love: 0, score: 0, wave: 1,
    phase: "playing", frameN: 0, uid: 0,
    heartT: 1.5, gremlinT: 2.0,
  };
}
...
// Pre-calculated pillar positions
...
export function GameScene({ joystickRef, fireRef, onHudUpdate }: Props) {
  const { camera } = useThree();
  const gs = useRef<GS>(initGS());

  // Only these drive re-renders (on spawn / death)
  ...
  useFrame((state, rawDelta) => {
    ...
    // ── Spawn hearts ──────────────────────────────────────────────────────────
    g.heartT -= dt;
    if (g.heartT <= 0 && g.hearts.length < 14) {
    ...
    // ── Spawn gremlins ────────────────────────────────────────────────────────
    g.gremlinT -= dt;
    if (g.gremlinT <= 0 && g.gremlins.length < 14) {
      g.gremlinT = Math.max(1.0, GREMLIN_INTERVAL_BASE - g.wave * 0.25);
    ...
    for (let i = g.hearts.length - 1; i >= 0; i--) {
      if (g.player.pos.distanceTo(g.hearts[i].pos) < PICKUP_R) {
        g.hearts.splice(i, 1);
        g.love = Math.min(100, g.love + LOVE_PER_HEART);
    ...
    if (fireRef.current) {
      ...
      if (g.love >= 100) {
        g.love = 0;
        // Auto-aim toward nearest gremlin
        ...
        if (nearest) {
          ...
          _dir.set(Math.sin(g.player.facing), 0, Math.cos(g.player.facing));
        }
        g.projs.push({
          id: uid(),
    ...
    for (const h of g.hearts) {
      ...
      if (m) {
        m.position.set(h.pos.x, 0.55 + Math.sin(t * 2.5 + h.bob) * 0.22, h.pos.z);
        m.rotation.y = t * 1.8 + h.bob;
      }
    }
    ...
    // Giant heart: pulse + HP color
    if (giantMesh.current) {
      giantMesh.current.scale.setScalar(1 + Math.sin(t * 2.4) * 0.06);
      const r = g.giantHp / GIANT_HP_MAX;
      (giantMesh.current.material as THREE.MeshLambertMaterial).color.setRGB(1, r * 0.1, r * 0.2);
    }

    // ── Camera smooth follow ──────────────────────────────────────────────────
    _camTgt.set(g.player.pos.x, 18, g.player.pos.z + 14);
    camera.position.lerp(_camTgt, Math.min(dt * 4, 1));
    camera.lookAt(g.player.pos.x, 0, g.player.pos.z);

    // ── HUD update (every other frame) ───────────────────────────────────────
    if (g.frameN % 2 === 0) {
      onHudUpdate({
        loveBar: g.love,
  ...
  return (
    <>
      ...
      <pointLight position={[0, 5, 0]} color="#ff3366" intensity={5} distance={18} decay={2} />
      ...
      <gridHelper args={[ARENA * 2, 24, "#280050", "#1a0035"]} />

      {/* Boundary pillars */}
      {PILLARS.map(([px, pz], i) => (
        <mesh key={i} position={[px, 1.5, pz]}>
          <cylinderGeometry args={[0.3, 0.45, 3, 8]} />
          <meshLambertMaterial color="#3a0060" />
        </mesh>
      ))}

      {/* ═══════════════════════════════════════════════════════
          GIANT HEART — the objective you are protecting
          ─────────────────────────────────────────────────────
          To use your own model:
            import { useGLTF } from '@react-three/drei';
            const { scene } = useGLTF(require('../../assets/models/giant-heart.glb'));
            replace the <mesh ref={giantMesh}> below with:
            <primitive ref={giantMesh} object={scene} position={[0,0,0]} />
          Target size: ~5 units wide
          ═══════════════════════════════════════════════════════ */}
      <mesh ref={giantMesh} position={[0, 1.5, 0]} castShadow>
      ...
      <mesh rotation={[-Math.PI / 2, 0, 0]} position={[0, 0.02, 0]}>
        ...
        <meshBasicMaterial color="#ff0044" transparent opacity={0.12} />
      </mesh>
      ...
      {/* ═══════════════════════════════════════════════════════
          ...
          ─────────────────────────────────────────────────────
          To use your own model:
            const { scene } = useGLTF(require('../../assets/models/player.glb'));
            replace <mesh ref={playerMesh}> with:
            <primitive ref={playerMesh} object={scene} />
          Bounding box: 1 × 1.5 × 0.8 units
          ═══════════════════════════════════════════════════════ */}
      <mesh ref={playerMesh} position={[0, 0.75, 8]} castShadow>
      ...
      {/* ═══════════════════════════════════════════════════════
          GREMLINS
          ─────────────────────────────────────────────────────
          Replace the group contents with:
            const { scene } = useGLTF(require('../../assets/models/gremlin.glb'));
            <primitive object={scene.clone()} />
          Bounding box: 0.8 × 1.4 × 0.8 units
          ═══════════════════════════════════════════════════════ */}
      {gremlinIds.map(id => (
        <group
          key={id}
      ...
      {/* ═══════════════════════════════════════════════════════
          COLLECTIBLE HEARTS
          ─────────────────────────────────────────────────────
          Replace <mesh> with your model:
            const { scene } = useGLTF(require('../../assets/models/heart-pickup.glb'));
            <primitive object={scene.clone()} />
          Bounding sphere radius: ~0.45 units
          ═══════════════════════════════════════════════════════ */}
      {heartIds.map(id => (
        <mesh
          key={id}
        ...
        >
          ...
          <meshLambertMaterial color="#ff3388" emissive="#ff1155" emissiveIntensity={0.7} />
        </mesh>
      ))}

      {/* Love-blast projectiles */}
      {projIds.map(id => (
        <mesh
          key={id}
          ref={(el: THREE.Mesh | null) => {
